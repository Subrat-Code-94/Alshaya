...
import { getTopUpAmount, getTopUpOptions, enableTopUpButton } from '../../scripts/giftcart/helper.js';
...
async function decorateOtpForm(form, egiftNumber, otpResponse, placeholders) {
...
async function sendOtpEmail(form, placeholders) {
  ...
  try {
    ...
    if (otpResponse.response_type === true) {
      await decorateOtpForm(form, egiftNumber, otpResponse, placeholders);
    } else {
      showErrorMessage(egiftInput, otpResponse.response_message);
    }
  } catch (error) {
    showErrorMessage(egiftInput, placeholders.egiftOtpError || 'An error occurred while sending OTP.');
  }
}
...
`).join('');
          ...
          <div class="egift-card-reload-amount">
            ...
            <div class="input-field-wrapper">
              <div class="input-wrapper input-field">
                <input type="number" class="egift-reload-amount" id="egiftReloadAmount" placeholder=" ">
  ...
  // Event listener to deselect reload options when custom amount is entered
  ...
  // Event listener to clear reload amount input when radio button is selected
  reloadAmountRadios.forEach((radio) => {
  ...
  reloadAmountButton.addEventListener('click', async (event) => {
    ...
    if (res.response_type === true) {
      const message = placeholders.topUpAddedCartMessage || 'Top up added to the cart.';
...
async function decorateUnlinkedForm(form, placeholders) {
  ...
  form.querySelector('#getCode').addEventListener('click', async (event) => {
    ...
    sendOtpEmail(form, placeholders);
  });
}
