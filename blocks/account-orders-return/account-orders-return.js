...
const getCardRefundMethodContent = async (
  ...
  if (order?.payment?.extension_attributes?.vault_payment_token?.token_details) {
    const {
      maskedCC,
      type,
    } = JSON.parse(order.payment.extension_attributes.vault_payment_token.token_details);

    cardLabel = (placeholders?.orderRefundCreditcardEnding || 'Credit Card ending {{}}').replace('{{}}', maskedCC);
    cardIcon = type.toLowerCase();
  }
...
const getAuraRefundMethodContent = async (placeholders) => {
  const label = placeholders?.orderRefundAura || 'Aura';
  const icon = 'aura';
...
const getRefundMethodsContent = async (order, placeholders) => {
  ...
  if (refundMethod === 'checkout_com_upapi') {
    // E-Gift
    const eGift = await getEGiftRefundMethodContent(order, placeholders, true, (data) => updateProcessedRmaDataObject('extension_attributes', data));
    if (eGift) {
      refundMethodsContent.appendChild(eGift);
    }

    // Card
    const card = await getCardRefundMethodContent(order, placeholders, true);
    if (card) {
  ...
  if (refundMethod === 'aura_payment') {
    const aura = await getAuraRefundMethodContent(placeholders);
...
const renderItemsToReturn = async (block, order, returnsConfig, placeholders) => {
  ...
  btnContinue.addEventListener('click', () => itemsToReturnContinueHandler(order));
