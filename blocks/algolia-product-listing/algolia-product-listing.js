...
function buildOutOfStockHTML(product, isWishlist = false) {
  let outOfStockHTML = '';
  if (isWishlist && product.gtm['gtm-stock'] !== 'in stock') {
...
function buildProductLabelHTML(product) {
  ...
  const prevPreInfo = getPreviewPreaccessType(prevPreData, productListingLanguage);
  if (prevPreInfo?.type) {
    const pLabel = getDetailsByType(product.product_labels[productListingLanguage], prevPreInfo.type);
    if (pLabel) {
      return `<div class="product-label label-preview ${pLabel.position} ">
                           ${pLabel.text} </div>`;
    }
  }
  return '';
...
export function updatePageLoader(response, placeholdersList, eventload = '', replaceProducts = false) {
  ...
  if (eventload === 'loadMore' && !replaceProducts) {
    dataLayerLoadMoreSelectionEvent(currentProductCount, totalProductCount);
  }
}
  ...
  response?.results[0]?.hits?.forEach(async (product, _, list) => {
    ...
    const htmlTemplate = `
        ...
        <div class="product-item-info">
            ...
            <a class="product-item-title" data-link='pdp' href="${productUrl?.replace('.html', '') || '#'}">
              <h6>#title#</h6>
            </a>
            #rating#
            #pricing#
    ...
    productItemDiv.querySelector('.wishlist-button-wrapper').addEventListener('click', async (event) => {
      ...
      updateWishlist(JSON.parse(currentProduct), wishListIconState).then(({ status, message }) => {
        ...
        if (status) {
          ...
          datalayerAddToWishlistEvent(!wishListIconState, product, !isWishlist ? 'plp' : 'wishlist');
        }
    ...
    productItemDiv.querySelectorAll('a[data-link="pdp"]').forEach((pdpLink) => {
      pdpLink?.addEventListener('click', () => {
        datalayerSelectItemListEvent(product, list.length);
  ...
  window.addEventListener('lazy-loaded', async () => {
    ...
    if (window.location.href.includes('/search?q=')) {
      if (totalItemsLength !== 0 && totalItemsLength > 0) {
        ...
        dataLayerInternalSearchEmptyEvent(totalItemsLength);
      }
    }
  }, { once: true });
...
function removeFilterFromUrl(filterAttr, filterKey) {
  ...
  let key = filterKey.toLowerCase().replace(/ /g, '__').replace(/\//g, '_').replace(/-/g, '_');
  ...
  if (COLOR_ATTRIBUTE.includes(attr)) {
    [key] = key.split(',');
  }
...
function writeFilterToUrl(filterAttr, filterKey) {
  ...
  if (filterAttr !== PRICE_ATTRIBUTE) {
    attr = filterAttr.replace('attr_', '');
    key = filterKey.toLowerCase().replace(/ /g, '__');
...
function checkUrlFacets() {
  ...
  facetsFoundArray.forEach((facet) => {
    ...
    const attrName = split[0];
    split = [...split.slice(1)];
    split.forEach((value) => {
      if (PRICE_ATTRIBUTE.includes(attrName)) {
        urlPrice = `${value} - ${parseInt(value, 10) + 5}`;
      } else if (SIZE_ATTRIBUTE.includes(attrName) && value.includes('_')) {
        urlFacets.push(`${attrName}-${safeEncodeURIComponent(value.replace(/_/g, '%2D'))}`);
        urlFacets.push(`${attrName}-${safeEncodeURIComponent(value.replace(/_/g, '%2F'))}`);
      } else {
        const val = value.replace(/__/g, '%20');
        urlFacets.push(`${attrName}-${val}`);
        urlFacets.push(`${attrName}-${safeEncodeURIComponent(val.replace(/_/g, '%2D'))}`);
        urlFacets.push(`${attrName}-${safeEncodeURIComponent(val.replace(/_/g, '%2F'))}`);
      }
    });
  });
}
...
async function handleResponse(response, categoryData) {
  ...
  if (
    ...
    stickyLogoContainer.append(logoOnStickyFilters);
    ...
    wishlistEl.innerHTML = '<span class="icon wishlist-icon"/>';

    // Minicart icon
    const cartEl = document.createElement('span');
    cartEl.setAttribute('aria-label', 'minicart-wrapper');
    cartEl.innerHTML = '<span class="minicart-quantity"></span><span class="icon cart-icon"/>';
