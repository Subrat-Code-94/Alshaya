...
function addPriceTagHandler(block) {
  ...
  document.addEventListener('mousemove', (e) => {
    if (isDragging) {
      ...
      if (positionData.innerText.includes(':')) {
        if (isMobileView) {
...
function pricePointerPositionUpdate(priceContents, isSidekickLibrary, block) {
  let tagPosition = '';
  let arrowPosition = '';
  const posArray = [];
  let priceTagElement = '';
  ...
  priceContents.forEach((priceContent) => {
    ...
    if (priceContent.querySelectorAll('p').length > 5) {
    ...
    tagPosition = `top:${y};left:${x};`;
    posArray.push(tagPosition);
    pricecontentMarkup += `<div class="price-content arrow-${arrowPosition.toLowerCase()}" style="${tagPosition}">
    ${priceContent?.outerHTML}
  </div>`;
  });
  if (block.querySelector('.banner-content-wrapper')) {
    block.querySelectorAll('.price-content').forEach((element, index) => {
      const styleResult = posArray[index % posArray.length];
      if (styleResult) {
        element.setAttribute('style', styleResult);
      }
    });
  }
}
...
export default async function decorate(block) {
  ...
  if (priceContents.length) {
    pricePointerPositionUpdate(priceContents, isSidekickLibrary, block);
