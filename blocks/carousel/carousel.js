 ...
 */
function handleCardClick(block, productData, totalProducts) {
  ...
  datalayerSelectRecommendedItemListEvent(productData, totalProducts, recommendationName);
}
...
export async function decorateDynamicCarousel(block, jsonData) {
  ...
  jsonData.forEach((products) => {
    ...
    const image = createOptimizedPicture(
      products?.productData?.image_url?.[document.documentElement.lang],
      false,
    ...
    anchor?.addEventListener('click', () => handleCardClick(block, products, jsonData.length));
