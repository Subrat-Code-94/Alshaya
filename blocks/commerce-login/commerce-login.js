...
async function login(form, redirectUrl) {
  ...
  try {
    ...
    if (!response.ok && data.message) {
      ...
      datalayerLogin('Login Attempt', 'Email : valid', 'ecommerce');
      return;
    }
    setCookie('auth_user_token', data, cookieExpiryDays);
    datalayerLogin('Login Success', 'Email');
...
export default async function decorate(block) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!validateForm(form)) {
      ...
      datalayerLogin('Login Attempt', `Email : invalid - ${errorFieldsList}`, 'ecommerce');
      return;
    }
