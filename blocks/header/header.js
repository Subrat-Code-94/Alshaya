...
import { logout, getWelcomeMessage } from '../../templates/account/account.js';
...
async function decorateHeader(block, placeholders, favouritesWidgetTimeout, fragment) {
    ...
    item?.addEventListener('click', (event) => {
      ...
      if (pageArray.length > 1) {
        ...
        const navLabels = finalPathArray.map((nav, index) => {
          const navPath = `/${selectedLanguage}/${finalPathArray.slice(0, index + 1).join('/')}`;
  ...
  document.querySelectorAll('.section.inline-links ul li a')?.forEach((link) => {
    ...
    link?.addEventListener('click', (event) => {
      ...
      if (link.href.endsWith('/ar/') || link.href.endsWith('/en/')) {
        event.preventDefault();
        const currentUrl = new URL(window.location.href);
        const newPathname = currentUrl.pathname.replace(/^\/[a-zA-Z]{2}/, `/${selectedLanguage}`);
        window.location.href = `${newPathname}${currentUrl.search}`;
      }
    });
  });
