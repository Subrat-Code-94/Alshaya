...
import { performCatalogServiceQuery, cacheProductData } from '../../../scripts/commerce.js';
import {
...
async function renderVariantSwatches(ctx, block, placeholders) {
  ...
  if (window.matchMedia('(min-width: 768px)').matches) {
    ...
    const actionsElement = block.querySelector('.pdp-product__actions');
    if (document.querySelector('#product-overview-popup') && actionsElement) {
      actionsElement.parentNode.insertBefore(swatchesList, actionsElement);
    } else {
