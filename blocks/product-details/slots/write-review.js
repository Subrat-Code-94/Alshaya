...
function uploadImage(input, placeholders, bvConfig) {
  ...
  if (input.files[0].size > 5 * 1024 * 1024) {
    imageError.textContent = placeholders.writeReviewPhotoUploadErrorFileSize || 'File size exceeds 5MB';
  ...
  if (!input.files[0].type.includes('image')) {
    imageError.textContent = placeholders.writeReviewPhotoUploadErrorFileType || 'File type is not an image';
    imageField.classList.add('error');
