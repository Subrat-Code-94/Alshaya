...
function DiscountAndVouchersLoggedinUser() {
  ...
  useEffect(() => {
    ...
    Promise.all([getHelloMemberOffers(), getHelloMemberCoupons()]).then(
      (values) => {
        const [offersResponse, couponsResponse] = values;

        if (offersResponse?.response?.message) setOfferError(offersResponse?.response?.message ?? '');
        else setAllOffers(offersResponse?.response?.offers ?? []);

        if (couponsResponse?.response?.error) setVoucherError(couponsResponse?.response?.message ?? '');
        else setAllVouchers(couponsResponse?.response?.coupons ?? []);
  ...
  const getUpdatedCart = async () => {
    const result = await getSubCartGraphql(isLoggedIn, cartId, [Constants.CART_QUERY__EXTENSION_ATTRIBUTE, Constants.CART_QUERY__PRICES]);
    ...
    <>
      ...
      <div className="discounts-vouchers-body">
              ...
              : (
                <>
                  ...
                  }
                  {
                    activeTab === offer
                      ? allOffers?.map((item) => (
                        <div key={`offer-${item?.code}`} id={`offer-${item?.code}`} className="item discounts-vouchers-items">
                          <div>
                            <input
                              type="radio"
                          ...
                          <div>
                            ...
                            <span className="expiry_date">
                              ...
                              {' '}
                              {modifyDate(item?.end_date)}
                            </span>
                          </div>
                        </div>
                      ))
                      : allVouchers?.map((item) => (
                        <div key={`voucher-${item?.id}`} id={`voucher-${item?.id}`} className="item discounts-vouchers-items">
                          <div>
                            <input
                              type="checkbox"
        ...
        <div className="popup-footer">
          ...
          <button type="button" className={`clear-button ${isClearAllDisabled ? 'clear-button-disabled' : ''} ${isRemoving ? 'loader' : ''}`} onClick={() => (activeTab === offer ? applyRemoveOffers(true) : applyRemoveVouchers(true))}>
