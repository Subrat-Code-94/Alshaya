...
function ProductSummaryCardGql({ product, currency, checkoutHideSection }) {
  ...
  const {
    cart, setCart, placeholders, cartId, priceDecimals, promotion,
  ...
  const renderProductOffers = () => {
    ...
    return promotions?.length ? (
      <div className="product-offer-wrapper">
        {
          promotions.map((promo) => <a key={`${product?.product?.name ?? ''}-${promo?.label ?? ''}`} href={`/${getLanguageAttr()}/${promo?.url ?? ''}`} className="product-offer">{promo?.label ?? ''}</a>)
        }
      </div>
    ) : null;
  };
  ...
  const renderImage = () => (isEgift ? ecartEstention?.image_url : productImageUrl);
  ...
  return (
    <div className="product-summary__card">
      <div className={`product-summary__container ${isLoading ? 'loader' : ''}`}>
        ...
        {!checkoutHideSection && (
          <div className="product-summary__actions">
            ...
            {((!isEgift && product.configured_variant?.stock_data?.qty) || isFreeGift) && (
      ...
      {!isEgift && ecartEstention?.error_message && (
        <div className="product-label__container">
          <span className="product-label">
            {ecartEstention?.error_message || placeholders?.outOfStockContent}
          </span>
        </div>
      )}
      {renderProductLabels()}
      {!isEgift && !isQtyAvailable && (
        <div className="product-summary__qtyNotAvailable">
          <span>{placeholders?.quantityNotAvailableLabel || 'The requested quantity is not available'}</span>
        </div>
      )}
    </div>
  );
