...
export const FULLNAME_PATTERN = String.raw`^[A-Za-z]+(?:\s[A-Za-z]+)+$`;
...
export async function validateAddressDropdowns(form, placeholders) {
...
export function validateForm(form, placeholders) {
  resetMessageForm();
  let isValid = true;
  const isCustomSelectAvailableInsideForm = form.querySelector('.custom-select');
  if (isCustomSelectAvailableInsideForm && !validateAddressDropdowns(form, placeholders)) {
    isValid = false;
  }
