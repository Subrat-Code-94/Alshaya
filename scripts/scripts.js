...
export async function getAndApplyRenderDecisions(payload, decisionScopes = ['__view__'], displayEvent = true, noDisplayScopes = []) {
  // Get the decisions, but don't render them automatically
  // so we can hook up into the AEM EDS page load sequence
  const xdm = await xdmPayload();
  const options = {
...
export async function fireTargetCall(payload, decisionScopes = [], fullPage = true) {
  let scopes = [];
  let tabScopes = [];
  ...
  return getAndApplyRenderDecisions(payload, [...scopes, ...decisionScopes], true, tabScopes);
}
