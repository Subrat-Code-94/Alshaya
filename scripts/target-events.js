...
import { getLoggedInUserAttributes, getPageAttributes } from './analytics/google-data-layer.js';
...
export async function pageLoadData() {
  ...
  const cartIds = currentPath.includes('/cart')
    ? productsInCart?.join(', ') || ''
    : undefined;
 ...
 */
export function targetDisplayEvent(payload = [], noDisplayScopes = []) {
...
export async function targetClickTrackingEvent({ key, recommendationName, personalizationCta }) {
  ...
  const [recommendationAttribs, personalizationAttribs, pageAttribs] = await Promise.all([
...
function createCategories(categoryList) {
  return {
    'entity.majorCategory': categoryList[0],
    'entity.minorCategory': categoryList[1],
    'entity.subCategory': categoryList[2],
    'entity.listingCategory': categoryList[categoryList.length - 1],
  };
}
...
export async function plpLoadData(categoryList = []) {
  ...
  const categoryObj = createCategories(categoryList);
